...
/*   Updated: 2025/04/09 20:46:50 by ajuliao-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool Server::cmdModeCheck(const std::vector<std::string>& args, int clientFd)
{
	...
	if (!cmdModeCheckArgs(args))
		return (notify(clientFd, WHI(isIRC), 2, 1, 1, "error : check '/help mode'"));

	Channel *channel = getChannelByName(args[0]);
	if (!channel)
		return (notify(clientFd, WHI(isIRC), 2, 1, 1, "can't set mode; channel doesn't exist"));

	Client* client = getClientByFd(clientFd);
	if (!channel->isClientOnChannel(client))
		return (notify(clientFd, WHI(isIRC), 2, 1, 1, "can't set mode; you haven't joined this channel"));

	if (!channel->isOperatorOnChannel(client) && channel->getOneMode("o"))
		return (notify(clientFd, WHI(isIRC), 2, 1, 1, "can't set mode; you're not channel's operator"));
	...
	for (size_t i = 1; i < args.size(); i++)
	{
		if (args[i] == "+o" && i + 1 < args.size() && checkAlphanum(args[i + 1], 0))
		{
			if (!channel->isClientOnChannel(getClientByNick(args[i + 1])))
				return (notify(clientFd, WHI(isIRC), 2, 1, 1, "can't add operator; this user didn't join to channel"));
		}
	}
...
void Server::cmdMode(const std::vector<std::string>& args, int clientFd)
{
	...
	for (size_t i = 1; i < args.size(); i++)
	{
		if (args[i] == "+k")
		{
			...
			notify(clientFd, WHI(isIRC), 6, 1, 1, cmdFlagAction(args[i]), channel);
			if (i + 1 < args.size() && checkAlphanum(args[i + 1], 0))
			{
